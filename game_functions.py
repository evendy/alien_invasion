...
def check_events(ai_settings, screen, stats, play_button, ship, aliens, bullets, scoreboard):
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            check_play_button(ai_settings, screen, stats, play_button, ship, aliens, bullets, scoreboard, mouse_x,
                              mouse_y)
...
def create_alien(ai_settings, screen, aliens, alien_number, row_number):
    """创建一个外星人并将其放在当前行"""
    alien = Alien(ai_settings, screen)
    alien.update_alien(alien_number, row_number)
    aliens.add(alien)
...
def ship_hit(ai_settings, stats, screen, ship, aliens, bullets, scoreboard):
    ...
    if stats.ships_left > 0:
        scoreboard.prep_life()
        reset_ship_aliens_bullets(ai_settings, screen, ship, aliens, bullets)
        # 暂停
        sleep(0.5)
    else:
        stats.game_active = False
        pygame.mouse.set_visible(True)
        stats.check_high_score(scoreboard)
...
def reset_ship_aliens_bullets(ai_settings, screen, ship, aliens, bullets):
    # 清空外星人列表和子弹列表
    aliens.empty()
    bullets.empty()
